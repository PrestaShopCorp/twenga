...
class TwengaFieldsGetTrackingScript extends TwengaFields
{
	public function __construct()
	{
		if (!is_array($this->fields) AND empty($this->fields))
		{
			...
			$this->fields['is_customer'] = array(56, array('isInt'));
			$this->fields['ecommerce_platform'] = array(56, array('is_string','isCleanHtml'));
			$this->fields['tag_platform'] = array(56, array('is_string','isCleanHtml'));
			$this->fields['event'] = array(32, array('is_string','isCleanHtml'), true);
			...
			// optionnals
			$this->fields['currency'] = array(40, array('is_string','isCleanHtml'), true);
			$this->fields['total_ttc'] = array(0, array('isFloat'));
			$this->fields['shipping'] = array(0, array('isFloat'), true);
			$this->fields['tax'] = array(0, array('isFloat'), true);
			$this->fields['tva'] = array(0, array('isFloat'), true);
			...
			$this->fields['cli_global_id'] = array(0, array('is_string','isCleanHtml'));
			$this->fields['cli_city'] = array(0, array('is_string','isCleanHtml'));
			$this->fields['cli_state'] = array(0, array('is_string','isCleanHtml'));
			$this->fields['cli_country'] = array(0, array('is_string','isCleanHtml'), true);
	...
	/**
	 * (non-PHPdoc)
	 * @see TwengaFields::checkParams()
	 */
	public function checkParams()
	{
		parent::checkParams();
		if(isset($this->fields['items']) && !empty($this->fields['items'])){
			self::isItemsOrder($this->fields['items']);
		}
	}
	public static function isItemsOrder ($value)
	{
		...
		foreach ($value as $key=>$item)
		{
			...
			$str_error .= ' :';
			if (isset($item['price']) && !Validate::isFloat($item['price']))
				throw new TwengaFieldsException($str_error.Tools::displayError('The price must be a float value.'));
			...
			if (isset($item['item_id']) && empty($item['item_id']))
				throw new TwengaFieldsException($str_error.Tools::displayError('The item_id must be a string with length less than 40 chars.'));
